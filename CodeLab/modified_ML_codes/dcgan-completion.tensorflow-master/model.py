...
def dataset_files(root):
    ...
    return list(itertools.chain.from_iterable(
        glob(os.path.join(root, img_dir, "*.{}".format('png'))) \
...
class DCGAN(object):
    def __init__(self, sess, image_size=64, is_crop=False,
        ...
        Args:
            ...
            lam: the proportion of the perceptual loss in completing the image. [0.1]
        """
        ...
        self.lowres = lowres  # [8]
        self.lowres_size = image_size // lowres  # [8]
        self.lowres_shape = [self.lowres_size, self.lowres_size, c_dim]  # [8, 8, 3]
    ...
    def build_model(self):
        ...
        self.images = tf.placeholder(
            tf.float32, [None] + self.image_shape, name='real_images')  #NHWC [None, 64, 64, 3]
        self.lowres_images = tf.reduce_mean(tf.reshape(self.images,
            [self.batch_size, self.lowres_size, self.lowres,
             self.lowres_size, self.lowres, self.c_dim]), [2, 4])  # [batch_size, 64, 64, 3] --> [64, 8, 8, 8, 8, 3]
        self.z = tf.placeholder(tf.float32, [None, self.z_dim], name='z')  #[None, 100]
        self.z_sum = tf.summary.histogram("z", self.z)

        self.G = self.generator(self.z)  # G生成图像
        self.lowres_G = tf.reduce_mean(tf.reshape(self.G,
            [self.batch_size, self.lowres_size, self.lowres,
             self.lowres_size, self.lowres, self.c_dim]), [2, 4])
        self.D, self.D_logits = self.discriminator(self.images)  # D判别真实图像

        self.D_, self.D_logits_ = self.discriminator(self.G, reuse=True)  # D判别G生成的图像
        ...
        self.d_loss_real = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits,
                                                    labels=tf.ones_like(self.D)))  # D判别真图为1
        self.d_loss_fake = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits_,
                                                    labels=tf.zeros_like(self.D_)))  # D判别G生成的图为0
        self.g_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits_,
                                                    labels=tf.ones_like(self.D_)))  # G生成的图像, 目的愚弄D

        # 用自定义函数编写
        # self.d_loss_real = binary_cross_entropy(self.D_logits, tf.ones_like(self.D))  # D判别真图为1
        # self.d_loss_fake = binary_cross_entropy(self.D_logits_, tf.zeros_like(self.D_))  # D判别G生成的图为0
        # self.g_loss = binary_cross_entropy(self.D_logits_, tf.ones_like(self.D_))  # G生成的图像, 目的愚弄D

        self.d_loss_real_sum = tf.summary.scalar("d_loss_real", self.d_loss_real)
        self.d_loss_fake_sum = tf.summary.scalar("d_loss_fake", self.d_loss_fake)

        self.d_loss = self.d_loss_real + self.d_loss_fake  # 最终的D_loss
        ...
        self.d_vars = [var for var in t_vars if 'd_' in var.name]  # 训练D更新的参数
        self.g_vars = [var for var in t_vars if 'g_' in var.name]  # 训练G更新的参数

        self.saver = tf.train.Saver(max_to_keep=1)

        # Completion.
        self.mask = tf.placeholder(tf.float32, self.image_shape, name='mask')  # [64, 64, 3]
        self.lowres_mask = tf.placeholder(tf.float32, self.lowres_shape, name='lowres_mask')  # [8, 8, 3]
        self.contextual_loss = tf.reduce_sum(
            tf.contrib.layers.flatten(
                tf.abs(tf.multiply(self.mask, self.G) - tf.multiply(self.mask, self.images))), 1)  # l1 norm 可以试试l2
        self.contextual_loss += tf.reduce_sum(
            tf.contrib.layers.flatten(
    ...
    def train(self, config):
        ...
        # 这里的sample_xxx 都是为了训练中每100轮验证一次用的
...
""")
    ...
    def complete(self, config):
        ...
        data = dataset_files(config.imgpath)[:500]
        nImgs = len(data)
        ...
        if config.maskType == 'random':
            ...
            mask = np.ones(self.image_shape)  # [64, 64, 3]
            sz = self.image_size
        ...
        for idx in xrange(0, batch_idxs):
            ...
            save_images(batch_images[:batchSz,:,:,:], [nRows,nCols],
                        os.path.join(config.outDir, 'before.png'))  # 存大图，原图
            masked_images = np.multiply(batch_images, mask)
            save_images(masked_images[:batchSz,:,:,:], [nRows,nCols],  # 存大图，中间有黑框的图
    ...
    def generator(self, z):
        with tf.variable_scope("generator") as scope:
            ...
            hs = [None]
            hs[0] = tf.reshape(self.z_, [-1, 4, 4, self.gf_dim * 8])  #[N, 4, 4, 64*8]
