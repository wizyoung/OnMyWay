...
class TextDataSet(DataSet):
  ...
  def __init__(self, common_params, dataset_params):
    ...
    self.record_queue = Queue(maxsize=10000)  # 等待队列
    self.image_label_queue = Queue(maxsize=512)  # 输出队列

    # 子元素也是list，存储图片路径和坐标信息
    self.record_list = []  
    ...
    # 设定为守护进程，也就是后台进程。主进程结束后，不论后台进程结束与否，统统结束
  ...
  def record_producer(self):
    ...
    """
    while True:
      # 队列每存 record_number 个样本一次(整体轮流一遍)，打乱一次样本
  ...
  def record_process(self, record):
    """record process 
    Args: 一张图的record, 输入格式为[img_path, xmin1, ymin1, xmax1, ymax1, cls1, xmin2, ymin2, xmax2, ymax2, cls2， ...] 
    Returns:
      image: 3-D ndarray
      labels: 2-D list [max_objects, 5] (xcenter, ycenter, w, h, class_num) class_num 为属于的分类的编号
      object_num:  total object number, int, = min(self.max_objects, len(record)), 为单张图片上的框数量
    """
