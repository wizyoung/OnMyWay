...
class SSN(torch.nn.Module):
    def __init__(self, num_class,
        ...
        self.num_segments = starting_segment + course_segment + ending_segment # 2 + 5 + 2 = 9
        self.starting_segment = starting_segment # 2
        self.course_segment = course_segment # 5
        self.ending_segment = ending_segment # 2
        self.reshape = True
    ...
    SSN Configurations:
        ...
        # 选 base model，载入权重，根据 modality 设定均值和方差
        self._prepare_base_model(base_model)

        # 输入通道数，就是 base_model 的 output_dim (最后一个卷积层的输出通道数)
    ...
    def _prepare_ssn(self, num_class, stpp_cfg):
        ...
        self.activity_fc = nn.Linear(self.stpp.activity_feat_dim(), num_class + 1)  # 多一个背景类, n+1 类输出
    ...
    def _prepare_base_model(self, base_model):
        if 'resnet' in base_model or 'vgg' in base_model:
            self.base_model = getattr(torchvision.models, base_model)(True)  # True 表示载入权重
            ...
            # 用非 rgb 图像时的 mean, std 设定
            if self.modality == 'Flow':
                self.input_mean = [0.5] # 对光流，强行设定 mean 为 0.5, 就是范围的一半, 暴力啊
    ...
    def _construct_flow_model(self, base_model):
        ...
        conv_layer = modules[first_conv_idx] # 第一个的卷积层
