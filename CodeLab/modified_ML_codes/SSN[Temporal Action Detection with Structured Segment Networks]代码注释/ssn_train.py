...
# 一些参数说明：
# args.init_weights: 要载入的预训练权重的路径
# args.kinetics_pretrain: 是否载入kinetics预训练权重
# args.modality: 输入模式，['Flow', 'RGB']
# args.arch: 载入的模型结构，['BNInception', 'InceptionV3']
# args.gpus: gpu list. 不指定时 torch.nn.DataParallel 会调用 list(range(torch.cuda.device_count())) 生成代替
# args.resume: 恢复训练时的权重路径
# args.batch_size: batch_size
# args.evaluate: 是否测试
# args.eval_freq: 多久测试一次测试集

def main():
    global args, best_loss
    args = parser.parse_args()

    # 返回 dict, 为 yaml 配置文件内容
    dataset_configs = get_configs(args.dataset)

    num_class = dataset_configs['num_class']
    stpp_configs = tuple(dataset_configs['stpp']) # (1,1,1)
    ...
    # 是否载入 Kinetics pretrained models
    ...
    crop_size = model.crop_size  # 就是 base_model 的 input_size，ResNet101 是 224
    scale_size = model.scale_size  # crop_size * 256 // 224
    input_mean = model.input_mean
    input_std = model.input_std
    policies = model.get_optim_policies()
    train_augmentation = model.get_augmentation()

    # 分布式处理, device_ids 为 List，如[0, 1, 2]
    ...
    # data_length 也就是 SSNDataSet中的 new_length 
    if args.modality == 'RGB':
        data_length = 1
    # 这些信息要考虑前后关联关系，因此要每个采样点多采几帧
...
def train(train_loader, model, act_criterion, comp_criterion, regression_criterion, optimizer, epoch):
    ...
    data_time = AverageMeter()  # 载入数据的时间
