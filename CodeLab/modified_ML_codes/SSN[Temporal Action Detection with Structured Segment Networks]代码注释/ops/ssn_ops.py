...
class StructuredTemporalPyramidPooling(torch.nn.Module):
    ...
    def __init__(self, feat_dim, standalong_classifier=False, configs=(1, (1,2), 1)):
        ...
        # sc 为 True 时，forward 返回的第一个值为 course_feature, 否则是三阶段的 stpp cat特征
        # course feature 的目的是为 act classifier 服务
        ...
        self.feat_multiplier = starting_mult + course_mult + ending_mult  # 1 + (1+2) + 1
        self.parts = (starting_parts, course_parts, ending_parts)  # 一共几段, 如 (1, (1, 2), 1)
        self.norm_num = (starting_mult, course_mult, ending_mult)  # 每部分分别几段, 如 (1, 3, 1)

    def forward(self, ft, scaling, seg_split):
        '''
        param:
            ft: base_model 的输出 base_out 
            scaling: aug_scaling 参数  
            seg_split: [starting_segments, starting_segments + course_segments, num_segments(start + course + ending)]
        '''
        x1 = seg_split[0]  # starting segments
        x2 = seg_split[1]  # course segments
        n_seg = seg_split[2]  # num_segments
        ft_dim = ft.size()[1]  # 输入的 base_out 的第一个参数, 是维度 C
        ...
        # 三阶段特征 f_s, f_c, f_e 在维度1上 concatenate
